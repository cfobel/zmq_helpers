...
from socket_group import SocketGroupDevice, ProcessSocketGroupDevice, DeferredSocket, get_echo_server, IOLoop
...
def get_consumer(delay=0):
    ...
    def process_input(self, stream, multipart_message):
        print '[CONSUMER:process_input] %s %s' % (stream, multipart_message, )
        ...
        print '[CONSUMER:process_input] req response:', response
...
def ___run_producer():
    ...
    def process_response(stream, multipart_message):
        print '[PRODUCER:process_response] %s %s' % (stream, multipart_message, )

    def process_request(self, stream, multipart_message):
        print '[PRODUCER:process_request] %s %s' % (stream, multipart_message, )
